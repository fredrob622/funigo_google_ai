...
app.post('/quiz/search', async (req, res) => {
    ...
    try {
        ...
        // CORRIGÉ : On fournit bien 1 valeurs pour niveau '?'
		// dbPool.query() prend une requête SQL et des paramètres, les envoie à la base de données, 
		// attend la réponse, et vous la retourne de manière sécurisée. (module mysql2)
		// dbPool n'est pas une simple connexion à la base de données, mais un "pool" (ou une "piscine") de connexions. 
		// c'est un objet, probablement créé au démarrage de votre application (avec une librairie comme mysql2), 
		// qui gère un ensemble de connexions prêtes à l'emploi.
...
// Crée une route POST à l’URL /quiz/search_question
// Elle attend que le client envoie des données (par ex. via fetch, axios, ou un formulaire JS)
...
app.post('/quiz/search_question', (req, res) => {
  ...
  // Le console.log va maintenant afficher la valeur dynamique !
  console.log('Numéro de la question reçu du client:', numeroQuestionRecu);
  
  // Vous pouvez maintenant utiliser cette variable pour faire une recherche en BDD, etc.
  // Par exemple, trouver la question qui a l'index `numeroQuestionRecu`.
  
  // On renvoie une réponse pour confirmer la réception
  res.json({ 
      success: true, 
      message: `Bien reçu le numéro de question ${numeroQuestionRecu}` 
      });
});
